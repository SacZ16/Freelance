...
export default function Home({ usuarioJWT, setActualizar, actualizar }) {
  ...
  useEffect(() => {
    fetch("https://free-q3yd.vercel.app/home", optionGet)
      .then((r) => r.json())
      .then((e) => {
        setHome(e[0]);
        console.log(e);
    ...
    <main>
      ...
      >
        ...
        <section className="grid-container-cards">
          {elegidos &&
            elegidos.slice(0, 4).map((producto, index) => {
              return (
                <Card
                  key={producto._id}
                  usuarioJWTFAV={
                    usuarioJWT
                      ? usuarioJWT.favoritos
                          .map((e) => e._id)
                          .includes(producto._id)
                      : false
                  }
      ...
      <section className="section-destacados-home">
        ...
        <section className="grid-container-cards">
          {home.destacados &&
            home.destacados.slice(0, 4).map((producto, index) => {
              return (
                <Card
                  key={producto._id}
                  id={producto._id}
